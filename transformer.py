...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        x = x + self.pe[:, :x.size(1), :].detach()  # make the parameter not learnable
...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        return x + self.dropout(sublayer(self.norm(x)))  # sublayer is the previous layer 
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):  # src_mask is the mask applied to the encoder and tgt mask is the mask applied to the decoder
